...
class MinimalSimConfig(n: Int = 1)
    extends Config(
...
class SuperMiniConfig(n: Int = 1)
    extends Config(
      new MinimalConfig(n).alter((site, here, up) => { case SoCParamsKey =>
        up(SoCParamsKey).copy(
          L3CacheParamsOpt = None
        )
      })
...
class WithNKBL1D(n: Int, ways: Int = 8)
    extends Config((site, here, up) => { case XSTileKey =>
            ...
            L2Param(
              ...
              clientCaches = Seq(
                L1Param(
                  ...
                  vaddrBitsOpt = Some(
                    p.VAddrBits - log2Up(p.dcacheParametersOpt.get.blockBytes)
                  )
                )
              ),
...
class WithNKBL3(
    ...
    inclusive: Boolean = true,
    banks: Int = 1
) extends Config((site, here, up) => { case SoCParamsKey =>
